...
final class HealthKitBridge: ObservableObject {
    ...
    private func writeBack(whoopStore: WhoopStore, days rollingDays: Int = 14) async throws {
        ...
        // A rewrite deletes before it writes, so nothing may be skipped as "unchanged" on this run.
        if historyRewrite || UserDefaults.standard.bool(forKey: IntelligenceEngine.restingHRHealthRewriteOwedKey) {
            for kind in ["vitals", "sleep", "workouts", "heartRateTail"] {
                UserDefaults.standard.removeObject(forKey: writtenBatchKey(kind))
            }
        }
    ...
    private func writeVitals(whoopStore: WhoopStore, days: Int, sessions: [CachedSleepSession],
        ...
        } + retracted.values.flatMap { $0 }.map { "retract|\($0)" })
        ...
        if !candidates.isEmpty { try await self.store.save(candidates.map { $0.sample }) }
