...
/// Fold the per-day diagnostic lines onto the one channel that already replays them.
    ...
    func computedHistorySpanDays() async -> Int {
        guard let store = await repo.storeHandle(),
              let rows = try? await store.dailyMetrics(deviceId: deviceId + "-noop", from: "0000-01-01", to: "9999-12-31"),
              let first = rows.map(\.day).min() else { return 4000 }
        let formatter = DateFormatter()
        formatter.dateFormat = "yyyy-MM-dd"
        formatter.timeZone = TimeZone.current
        guard let firstDate = formatter.date(from: first) else { return 4000 }
        let days = Calendar.current.dateComponents([.day], from: firstDate, to: Date()).day ?? 4000
