...
final class ChargeDriversTests: XCTestCase {
    ...
    func testDriverPointRoundingUsesNearestWithHalfTiesAwayFromZero() {
        ...
        let negativeBaseline = BaselineState(
            baseline: 30.0, spread: 0.5512100000000001, nValid: 14,
            nightsSinceUpdate: 0, status: .trusted)
        // Fork: the magic inputs (and this spread) are re-solved for the fork's `wRHR`/`logisticK`/
        // `logisticZ0` so each tie still lands exactly on ±0.5.
        let negativeBelowTie = hrvMarginal(
            hrv: 29.98792611159785, rhr: 60.0, hrvBaseline: negativeBaseline)
        let negativeTie = hrvMarginal(
            hrv: 29.987926111597847, rhr: 60.0, hrvBaseline: negativeBaseline)
        let negativeBeyondTie = hrvMarginal(
            hrv: 29.987926111597844, rhr: 60.0, hrvBaseline: negativeBaseline)
    ...
    func testIssue51NegativeHalfTieUsesDefaultArg8WithoutChangingScoreOrDriverFields() {
        ...
        let scoreBefore = RecoveryScorer.recovery(
            hrv: 29.987926111597847, rhr: 60.0, resp: nil,
